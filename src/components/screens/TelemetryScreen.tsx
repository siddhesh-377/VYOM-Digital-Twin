    ...
    const history = rawHistory.length >= 5
      ...
      : Array.from({ length: 25 }, (_, i) => {
          ...
          return {
            ...
            power: {
              batteryPercent: 96.4 + Math.sin(i * 0.4) * 0.8,
              voltageV: 28.6 + Math.cos(i * 0.3) * 0.1,
              currentA: 4.2,
              solarGenerationW: 260 + Math.sin(i * 0.5) * 10,
              consumptionW: 120 + Math.cos(i * 0.2) * 5,
            },
            thermal: {
              cpuTempC: 41.8 + Math.sin(i * 0.3) * 0.6,
              batteryTempC: 18.2 + Math.cos(i * 0.2) * 0.3,
              payloadTempC: 32.5 + Math.sin(i * 0.4) * 0.4,
              externalTempC: -15.0,
            },
            comm: {
              signalDbm: -72 + Math.sin(i * 0.6) * 2,
              dataRateMbps: 8.4 + Math.cos(i * 0.3) * 0.4,
              packetsPerSec: 240,
              latencyMs: 340,
              uptime: 100,
            },
            attitude: {
              rollDeg: 0.12 + Math.sin(i * 0.2) * 0.05,
              pitchDeg: -0.08 + Math.cos(i * 0.2) * 0.04,
              yawDeg: 0.04,
              angularVelDegs: 0.01,
              reactionWheelRpm: 3240 + Math.sin(i * 0.5) * 40,
            },
            compute: {
              cpuPercent: 32 + Math.sin(i * 0.4) * 4,
              memoryPercent: 48,
              storagePercent: 12.5,
            },
            orbit: {
