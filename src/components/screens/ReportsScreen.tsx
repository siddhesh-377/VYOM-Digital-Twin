...
type ReportTab = 'summary' | 'health-integrity' | 'incidents' | 'milestones';
...
export function ReportsScreen() {
  ...
  const powerData = useMemo(() => {
    ...
    return Array.from({ length: 30 }, (_, i) => {
      const offset = (30 - i) * 0.15;
      return {
        index: i,
        timeLabel: `T-${(30 - i) * 2}s`,
        batteryPercent: Math.max(0, Math.min(100, Number((currentBat - Math.sin(offset) * 1.5).toFixed(1)))),
        solarWatts: Math.max(0, Math.round(currentSolar + Math.cos(offset) * 15)),
        consumptionWatts: Math.max(0, Math.round(currentCons + Math.sin(offset * 0.5) * 8)),
        voltageV: Number((currentVolt - 0.05 * Math.sin(offset)).toFixed(2)),
      };
    });
