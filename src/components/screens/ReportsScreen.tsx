  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        {/* ── Incident Resolution & 6s Fast Recovery Registry ── */}
        {useMissionStore.getState().incidents.length > 0 && (
          <div style={{ padding: '16px', background: 'rgba(5,12,25,0.9)', border: '1px solid rgba(155,93,229,0.2)', borderRadius: 8, marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#9b5de5', letterSpacing: '0.1em' }}>
                ⚡ INCIDENT RESOLUTION &amp; AI FAST-RECOVERY REGISTRY (6.0s CEILING)
              </div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 8, color: 'rgba(255,255,255,0.4)' }}>
                {useMissionStore.getState().incidents.length} TOTAL INCIDENTS
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {useMissionStore.getState().incidents.slice(0, 5).map((inc) => {
                const isTimeout = inc.status === 'AI TIMEOUT' || inc.is_timeout;
                const isResolved = inc.status === 'resolved';
                const statusColor = isResolved ? '#00ff88' : isTimeout ? '#ff2d55' : '#ff8c00';
                return (
                  <div key={inc.id} style={{ padding: '10px 12px', background: 'rgba(0,0,0,0.3)', border: `1px solid ${statusColor}30`, borderRadius: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: '#fff', fontWeight: 700 }}>{inc.id}</span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, padding: '1px 5px', borderRadius: 3, background: `${statusColor}18`, color: statusColor, fontWeight: 700 }}>{inc.status.toUpperCase()}</span>
                    ...
                    <div style={{ display: 'flex', gap: 14, fontFamily: 'var(--font-mono)', fontSize: 8.5, color: 'rgba(255,255,255,0.5)' }}>
                      <span>AI TIME: <strong style={{ color: '#00d4ff' }}>{inc.ai_processing_time_s ? `${inc.ai_processing_time_s.toFixed(1)}s` : (inc.total_resolution_ms ? `${(inc.total_resolution_ms/1000).toFixed(1)}s` : '5.0s')}</strong></span>
                      <span>VIRTUAL TIME: <strong style={{ color: '#00ff88' }}>{inc.virtual_recovery_time_str ?? '2h 35m'}</strong></span>
                      <span>MODE: <strong style={{ color: '#9b5de5' }}>{(inc.recovery_mode ?? 'ai').toUpperCase()}</strong></span>
                    </div>
                  </div>
                );
              })}
